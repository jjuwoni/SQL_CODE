...
/*
	...
	* 데이터의 일관성이 지켜지지 않았다고 표현을 한다.
    ...
    트랜잭션은 두 개의 update 구문을 하나의 논리적인 작업 단위로 묶어, 두 작업이 모두 성공하거나, 또는 하나라도 실패를 하게 되면 원상복구 하도록 보장하는 기능이다.
    
    이 트랜잭션의 기능으로 데이터베이스는 어떤 상황이라도 데이터의 무결성과 일관성을 지킬 수 있다.
    
*/

-- start transaction : 지금부터 시작되는 모든 SQL 작업을 하나로 묶는 명령어
-- commit : 트랜잭션 내의 모든 작업이 성공적으로 완료 되면 DB에 영구 보관
