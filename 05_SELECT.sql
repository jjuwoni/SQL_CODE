...
-- where 를 얼마나 잘 사용하느냐에 따라 SQL 의 성능과 정확성이 결정된다.

-- users 테이블에서 user 의 status 가 active 가 아닌 모든 사용자의 user_id 와 status 조회
/*
	비교 연산자
    = : 같다
    != , <> : 같지 않다
...
-- users 테이블에서 user_id가 10 보다 큰 사용자 조회
select *
from users
where user_id > 10;

-- 24년 02 월 01 일 에 가입한 회원을 모두 조회하라
-- between a and b 사이 조건
select *
from users
where created_at between '2024-02-01 00:00:00' and '2024-02-01 23;59;59';

-- 목록 조건(IN)
-- user_id 가 1,5,10번인 사용자 정보 조회
select *
from users
...
-- 2024년 3월 이후에 가입을 했고, Status 가 Active 인 사용자의 이름과 이메일 조회
select name, email
from users
...
-- limit는 '상위 5개만 보기', 1페이지에 10개씩 보기' 등
-- 웹 어플리케이션의 페이징 기능을 구현하는데 필수 핵심 문법이다.

-- users 테이블에서 아이디, 이름, 가입일을 조회할건데 가장 최근에 가입한 순서대로 나열
...
-- 1. 2024년 4월 또는 5월에 가입한 사용자여야한다.
-- 2. 계쩡의 상태가 활서오하 상태여야 한다.
-- 3. 이메일 주소가 'example.com' 으로 끝나야 한다.
-- 4. 위 조건에 맞는 사용자들을 가장 최근에 가입한 순서대로 정렬하여, 
-- 5. 두번째 페이지에 해당하는 5명(6~10번째의 user_id, name, email, crerated_at)을 조회하라
