...
use wanted_lms;
-- ~ ` 백틱 으로 기존에 사용중인 예약어가 있다면 감싸서 사용을 할 수 있다.
INSERT INTO users (`email` , `name`) VALUES ('email@gmail.com', '서주원');
INSERT INTO users (`email` , `name`) VALUES ('email2@gmail.com', '서주순');
...
-- 에러의 원인은 courses 테이블의 author_id 는 실제 users 테이블의 users_id 를 참조하는 foreign key 제약조건을 가지고 있다.
INSERT INTO courses (`author_id`, `title`, `description`)
...
-- EX) 이학습 학생의 이름을 개명하거나, 과정의 설명이 변경 될 때 사용한다.

-- 테이블 정보 확인용 구문
SELECT * FROM users;

-- UPDATE 시 가장 유의해야 하는 점
-- WHERE 조건절을 생략을 하게 되면, 테이블의 모든 행이 SET 한 값으로 변경되는 대참사가 발생한다.
...
-- 사용자가 탈퇴하거나, 게시글을 삭제 했거나, 과정이 폐강 될 때 사용한다.

-- DELETE 절 사용시 주의점
-- DELETE 절에서 WHERE 조건절을 작성하지 않으면 모든 데이터가 삭제된다.
-- 따라서 UPDATE 구문과 마찬가지로 SELECT 를 통해 삭제 대상응ㄹ 우선 탐색하는 게 매우 중요하다.
DELETE FROM users
WHERE email = 'email@gmail.com';

-- 에러케이스
-- user_id = 1 인 사용자는 courses 테이블에서 참조하고 있다.
-- users 테이블에서 courses 테이블에 user_id 컬럼을 fk 로 주고 있다.
-- 자식 테이블(courses) / 부모 테이블(users)
-- 부모 테이블에 값을 삭제하려면, fk 제약조건에 의해 삭제가 거부된다.
