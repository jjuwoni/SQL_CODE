...
-- 테이블을 나누는 것은 정규화 라고 하며, 정규화에 의해 비슷한 종류의 데이터끼지 테이블을 구성하게 된다.

-- join 은 이렇게 분리 된 테이블들 중 공통으로 포함하는 값(주로 PK-FK) 을 기준으로 다시 합쳐서 의미 있는 결과 집합을 만들어낸다.

-- inner join (두 테이블의 교집합 찾기)
...
-- courses 테이블, users 테이블을 조합하여 과정 제목과, 그 과정을 만든 교육자 이름을 조회하라.
select c.title as '과정 제목', u.name as '교육자 명'
from courses as c
inner join users as u on c.author_id = u.user_id;

-- 테이블 3개 조인하기
-- enrollments 테이블 기준으로 users, courses 를 연결하여 어떤 학생이 어떤 과정에 등록했는 지 학생 이름과 과정 제목과 등록일을 조회하라.
select name as '학생 이름', title as '과정 제목', e.enrolled_at as '등록일'
...
-- right join
-- left jon 과 반대로, 오른쪽 테이블의 모든 데이터를 우선적으로 기준 삼는다.
-- 왼쪽 테이블의 경우 매칭되는 데이터가 없으면, null 로 표기된다.

-- user 테이블 기준, us_auth_providers 를 right join 하여
...
-- self join 
-- 하나의 테이블ㄹ 안에 계층적인 관계가 포함될 때
-- 예를 들어, 댓글이 있고 대댓글이 있다라고 가정을 하면, 댓글과 대댓글 또한 댓글이다.
-- 하지만 대댓글의 경우 부모 댓글에 대한 정보를 가져야 어떤 댓글의 대댓글인 지를 확인할 수 있다.

-- forum_posts 테이블을 사용해서 self join 진행.
-- 대댓글, 댓글내용을 함께 조회
select parent.post_id as '원본 글 아이디', child.post_id as '대댓글 아이디',
       parent.content as '원본 글 내용', child.content as '대댓글 내용'
