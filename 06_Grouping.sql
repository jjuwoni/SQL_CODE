...
select count(*) as '총 사용자 수'
from users;

-- user_profiles 테이블에서 bio(자기소개) 를 작성한 사용자는 몇 명인지 조회
select count(*) as '자소서 작성자 수'
from user_profiles;

-- courses 테이블에 과정을 등록한 교육자가 총 몇 명인지 중복 없이 계산 후 조회
select * from courses;
...
select sum(score) as '총점', avg(score) as '평균 점수'
from grades;

select max(score) as '최고', min(score) as '최저'
from grades;

-- [에러케이스] 비집계 컬럼과 집계함수 동시 사용 시 발생
-- 집계함수는 여러 행의 값을 하나의 결과로 요약한다.
-- 하지만 select 절에 집계 함수와 함계 Group by 절 없이 일반 컬럼(비집계)을 함께 명시하게 되면, 어떤 name 값을 cout(*) 와 함께 묶어야 할 지 알 수 없어 에러가 발생한다.
select name, count(*) as '총 사옹자수'
from users;

-- Group By
-- 테이블의 데이터를 특정 컬럼을 기준으로 그룹을 묶고, 각 그룹에 대해 집계 함수를 적용하여 소계를 계산하는 기능이다. 
...
-- courses 테이블에서 각 교육자가 몇 개의 과정을 개설했는 지 조회하라
select * from courses;
...
-- 2024 년에 가입한 사용자들 중에서, 월별 가입자 수가 15명 이상인 월과 해당 월의 가입자 수를 조회한다.
-- users 테이블에서 진행.

-- 문제 상황 제기
-- 월별로 데이터를 그룹화 해야 하는데
-- 실제 creted_at 컬럼의 데이터는 일과 시간도 작성 되어 있다.
